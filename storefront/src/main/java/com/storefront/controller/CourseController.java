...
public class CourseController {
	...
	public @ResponseBody ResponseEntity<List<Course>> getAllCourses(OAuth2Authentication authentication)
			throws StoreFrontException {
		try {
			return new ResponseEntity<List<Course>>(iCourseManagement.getAllPublishedCourses(authentication), HttpStatus.OK);
		} catch (Exception exception) {
			logger.info("Error while fetching course list: ", exception);
			return null;
		}
	}
	...
	public @ResponseBody ResponseEntity<Course> getCourseByCourseId(@PathVariable Long courseId,
		...
		try {
			...
			return new ResponseEntity<Course>(iCourseManagement.findCourseByCourseId(courseId, details.getTokenValue()),
					HttpStatus.OK);
		} catch (Exception exception) {
	...
	public @ResponseBody ResponseEntity<List<Course>> getCoursesByAuthor(OAuth2Authentication authentication)
			throws StoreFrontException {
		try {
			return new ResponseEntity<List<Course>>(iCourseManagement.getAllCourseByAuthor(authentication),
					HttpStatus.OK);
		} catch (Exception exception) {
	...
	public @ResponseBody ResponseEntity<Course> enrollCourse(@RequestBody Map<String, Object> payload,
		...
		try {
			return new ResponseEntity<Course>(
					iCourseManagement.enrollCourse(payload.get("courseId").toString(), authentication), HttpStatus.OK);
		} catch (Exception exception) {
			logger.info("Error while configurating course as preview course: ", exception);
			return null;
		}
	...
	public @ResponseBody ResponseEntity<Course> bookMarkCourse(@PathParam("courseId") String courseId,
		...
		try {
			return new ResponseEntity<Course>(
					iCourseManagement.bookMarkCourse(courseId, Boolean.parseBoolean(isBookMarked), authentication),
					HttpStatus.OK);
		} catch (Exception exception) {
			logger.info("Error while configurating course as preview course: ", exception);
			return null;
